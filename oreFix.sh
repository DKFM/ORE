...
PRIORITY_FEE=${custom_priority_fee:-100}
...
# 获取用户输入的序号
read -p "请输入启动序号: " ore_number
number=${ore_number:0}

# 使用 screen 和 Ore CLI 开始挖矿
session_name="ore_$number"
echo "开始挖矿，会话名称为 $session_name ..."
...
# 启动命令模板，使用变量替代rpc地址
start_command_template="while true; do ore --rpc $rpc_address --keypair ~/.config/solana/claim.json --priority-fee 100 claim; echo '进程异常退出，等待重启' >&2; sleep 1; done"
